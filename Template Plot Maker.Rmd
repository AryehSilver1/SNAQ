...
barC = ggplot(Cbargraph, aes(x = identity, y = prop, fill = Cplusorminus)) +
  ...
  labs(title = paste0("Proportion of ", markerC, "+ and ", markerC, "- for each cell type"),
       ...
       y = "Proportion",
       fill = "Identity") +
  geom_text(aes(label = paste(round(prop, 3))), position = position_fill(reverse = FALSE), vjust = 2, color = "black") +
...
plotClosestB = ggplot(data = closestB, aes(x = category, y = dist, fill = category)) +
  ...
  labs(title = paste0("Average distance of the closest ", markerB, " from each ", markerA,  " or ",  markerD),
       x = "",
       y = "Distance (µm)") +
  geom_text(data = medians, aes(label = round(dist, 2)), vjust = -0.5) +
  theme(legend.position = "none")
...
plotClosest10B = ggplot(data = closest10B, aes(x = category, y = dist, fill = category)) +
  ...
  labs(title = paste0("Average distance of the closest 10 ", markerB, " cells from each ", markerA, " or ", markerD),
       x = "",
       y = "Distance (µm)") +
  geom_text(data = medians, aes(label = round(dist, 2)), vjust = -0.5) +
  theme(legend.position = "none")
...
plotClosestB.C = ggplot(data = closestB.C, aes(x = category, y = dist, fill = category)) +
  ...
  scale_x_discrete(limits = c("AdistToB.C", "DdistToB.C"), labels = c("AdistToB.C" = paste0("distance from ", markerA, " to ", markerC, "+ ", markerB), "DdistToB.C" = paste0("distance from ", markerD, " to ", markerC, "+ ", markerB))) +
    labs(title = paste0("Average distance of the closest ", markerC, "+ ", markerB, " from each ", markerA, " or ", markerD),
       x = "",
       y = "Distance (µm)") +
  geom_text(data = medians, aes(label = round(dist, 2)), vjust = -0.5) +
  theme(legend.position = "none")
